...
本篇文章具体讲解`SpringBoot`与`Shiro`的整合操作，同时对于后端数据库中数据的获取使用到`MyBatis`,最后结合`shiro`与`Thymeleaf`完成对于不同用户的登录之际进行不同的展示信息。
...
创建一个`SpringBoot`项目和其三层架构关系就不再进行具体的讲解，具体架构如下：

![image-20200814220812056](C:\Users\ASUS\Desktop\upload\image-20200814220812056.png)
